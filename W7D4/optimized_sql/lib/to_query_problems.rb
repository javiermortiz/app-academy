...
def no_apples_for_blair_sub
  ...
  JOIN (SELECT * 
        ...
        WHERE toys.name = 'Apple') AS sub
  ON sub.cat_id = cats.id
  WHERE cats.name != 'Blair'
  ORDER BY cats.name;
  SQL
end
...
def toys_that_brendon_owns_sub
  ...
  execute(<<-SQL)
  SELECT sub.name
  FROM (SELECT toys.name
        FROM cats
        JOIN cattoys
        ON cattoys.cat_id = cats.id
        JOIN toys
        ON toys.id = cattoys.toy_id
        WHERE cats.name = 'Brendon') AS sub
  ORDER BY 1;
  SQL
end
...
def price_like_shiny_mouse
  ...
  FROM toys
  WHERE toys.name != 'Shiny Mouse' AND price BETWEEN 5 AND 9
  ORDER BY 1;
  SQL
end
...
def price_like_shiny_mouse_sub
  ...
  WHERE toys.price IN (SELECT toys.price
                      FROM toys
                      WHERE toys.name = 'Shiny Mouse') AND toys.name != 'Shiny Mouse'
  ORDER BY 1;
  SQL
end
...
def just_like_orange
  ...
  SELECT name, breed
  FROM cats
  WHERE breed = 'Brazilian Shorthair' AND name != 'Orange'
  ORDER BY 1;
  SQL
end
...
def toys_that_jet_owns
  ...
  JOIN toys
  ON toys.id = cattoys.toy_id
  WHERE toys.id IN (3301,3302,3303,3304,3305,3306,3307,3308,3309,3310,3311,3312,4961,3370,2019,4464,10,9271,8271,711,71) AND cats.name != 'Jet'
  ORDER BY 1;
  SQL
end
...
def toys_that_jet_owns_sub
  ...
  JOIN toys
  ON toys.id = cattoys.toy_id
  WHERE toys.id IN (SELECT cattoys.toy_id FROM cattoys WHERE cat_id = 1) AND cats.name != 'Jet'
  ORDER BY 1;
  SQL
end